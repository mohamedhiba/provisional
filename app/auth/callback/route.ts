import { cookies } from "next/headers";
import { NextRequest, NextResponse } from "next/server";

import { createAuthCompletePath, sanitizeNextPath } from "@/lib/auth-flow";
import { onboardingDeviceCookieKey } from "@/lib/onboarding";
import { attachAuthenticatedProfile } from "@/lib/profile-repository";
import { createSupabaseServerClient } from "@/lib/supabase/server";

export async function GET(request: NextRequest) {
  const requestUrl = new URL(request.url);
  const code = requestUrl.searchParams.get("code");
  const nextPath = sanitizeNextPath(requestUrl.searchParams.get("next"));
  const redirectUrl = new URL(
    createAuthCompletePath({
      status: "error",
      nextPath,
    }),
    request.url,
  );

  if (!code) {
    return NextResponse.redirect(redirectUrl);
  }

  try {
    const supabase = await createSupabaseServerClient();
    const { data, error } = await supabase.auth.exchangeCodeForSession(code);

    if (error || !data.user) {
      return NextResponse.redirect(redirectUrl);
    }

    const cookieStore = await cookies();
    const deviceId = cookieStore.get(onboardingDeviceCookieKey)?.value ?? null;
    const attachment = await attachAuthenticatedProfile({
      authUserId: data.user.id,
      deviceId,
    });

    const successRedirectUrl = new URL(
      createAuthCompletePath({
        status: "success",
        nextPath,
        mode: attachment.status,
      }),
      request.url,
    );

    return NextResponse.redirect(successRedirectUrl);
  } catch {
    return NextResponse.redirect(redirectUrl);
  }
}
